...
export class OAuth2TwitterService {
     ...
     */
    private async exchangeCodeForTokens(code: string, codeVerifier: string, redirectUri: string): Promise<TwitterTokens> {
        try {
            ...
            const refreshToken = typeof result === 'string' ? undefined : result.refreshToken;
            const expiresIn = typeof result === 'string' ? 7200 : result.expiresIn;
            const scope = typeof result === 'string' ? [] : (result.scope || []);
